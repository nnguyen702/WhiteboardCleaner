...
void StartPos(int dir_y, int pul_y, int dir_x, int pul_x, int dist_y, int dist_x) {

  // Vertical Direction
  if ((dist_y >= 74 && dist_y <= 76) || (dist_y <= 6)) {
...
void pos1_st(int dir_y, int pul_y, int dir_x, int pul_x, int dist_y, int dist_x) {
  //Vert. Dir
...
bool pos1_st_bool(int dir_y, int pul_y, int dir_x, int pul_x, int dist_y, int dist_x) {
  //Vert. Dir
  if (((dist_y >= 74) && (dist_y <= 76)) && ((dist_x <= 253) && (dist_x >= 245))) {
...
int clean1_pattern() {
  ...
  switch (State) {  //State actions
    case START:
      ...
      distX = getDistance(echoX, trigX);
      distY = getDistance(echoY, trigY);
      if (pos1_st_bool(dirY, pulY, dirX, pulX, distY, distX) == true) {
        num = 1;
      } else {
        num = 0;
      }
...
void pos2_st(int dir_y, int pul_y, int dir_x, int pul_x, int dist_y, int dist_x) {
  //Vert. Dir
...
bool pos2_st_bool(int dir_y, int pul_y, int dir_x, int pul_x, int dist_y, int dist_x) {
  //Vert. Dir
  if (((dist_y >= 74) && (dist_y <= 76)) && ((dist_x <= 206) && (dist_x >= 204))) {
...
enum cleaning2_States { START_p2,
                        wait_p2,
                        wipe_right_p2,
                        down_1_p2,
                        wipe_left_p2,
...
int clean2_pattern() {
  ...
  switch (State_p2) {  //Transitions
    ...
    case START_p2:
      ...
      if (num_p2 == 1) {
        ...
        State_p2 = START_p2;  //Initial state
      }
      break;
    case wipe_right_p2:
  ...
  switch (State_p2) {  //State actions
    case START_p2:
      ...
      pos2_st(dirY, pulY, dirX, pulX, distY, distX);
      distX = getDistance(echoX, trigX);
      distY = getDistance(echoY, trigY);
      if (pos2_st_bool(dirY, pulY, dirX, pulX, distY, distX) == true) {
        num_p2 = 1;
      } else {
        num_p2 = 0;
      }
